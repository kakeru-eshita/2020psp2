...
## ソースコードの説明
1.8:N-1人目までの平均とN人目の平均を足して、N人目までの平均を出したものを返す。

1.10:分散を求める関数名を定義する。

1.12:公式に当てはめN人目までの分散を出して返す。

1.20:N=0でNに0を代入する。

1.21:ave=0でave=0を代入する。

1.22:分散、二乗の平均、不偏分散をdouble型で定義する。
...
- プロトタイプ宣言にはexternをつけますが，関数本体にはつけません．
  - main関数の前に置くのはプロトタイプ宣言でexternをつけた関数本体の1行目
  - main関数の後ろに関数本体を置きます．externはつけません．
- 出力は，「平均」「分散」「不偏分散」ではなく，「標本平均」「標本分散」「母集団平均（推定値）」「母集団分散（推定値）」です．不偏分散というのは母集団分散の最良推定値を計算する方法にしかすぎません．レポートに出力結果の項を含めました．実行結果をコピペしてください．
- `gcc -o k00 k00.c -W -Wall`のように`-W`と`-Wall`のオプションをつけてコンパイルしてみてください．Warningが出てきたら，それが出ないようにソースコードを修正してください.  


[comment #20200612]
- OKです
- 入出力結果ですが, マークダウンでは勝手に改行してくれないので改行してほしいときは行末にスペースを2つ入れるようにしましょう
  - もしくは, バッククウォート3つで囲むと枠で囲んでくれて見やすくなると思います(変更しておきました)

[comment #20200619] (すいません, 修正追加です)
- プロトタイプ宣言をmain関数の前, 関数本体をmain関数の後に置くようにしてください. 
